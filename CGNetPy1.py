...
class ContextGuidedBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dilation=2, reduction=16, down=False,
        ...
        if down:
            self.conv = _ConvBNLeakyReLU(in_channels, inter_channels, 3, 2, 1, norm_layer=norm_layer, **kwargs)
            self.reduce = nn.Conv2d(inter_channels * 2, out_channels, 1, bias=False)
        else:
            self.conv = _ConvBNLeakyReLU(in_channels, inter_channels, 1, 1, 0, norm_layer=norm_layer, **kwargs)
    ...
    def forward(self, x):
        ...
        joi_feat = self.bnleakyrelu(joi_feat)
